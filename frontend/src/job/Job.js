...
function Job({ job }) {
    ...
    const [isAppliedTo, setAppliedTo] = useState(user ? user.applications.includes(id) : false);
    ...
    return (
        <Card>
            <CardBody>
                ...
                {user ?
                    (isAppliedTo ?
                        <Button className="mt-2">Applied</Button>
                        : <Button className="mt-2" color="primary" onClick={onClickApply}>Apply</Button>)
                    : ""}
            </CardBody>
        </Card>
    )
